...
public async initialize(config: RazorpayConfig): Promise<void> {
  ...
  public async listUserPayments(userId: string, filters?: ListPaymentsFilter) {
    ...
    try {
      const params: any = {
        from: filters?.fromDate ? new Date(filters.fromDate).getTime() / 1000 : undefined,
        to: filters?.toDate ? new Date(filters.toDate).getTime() / 1000 : undefined,
        count: 100,
      }
      const allPayments = await this.razorpay.payments.all(params)
      const results = allPayments.items.filter(
        (p: any) =>
          p.notes && p.notes.userId === userId &&
