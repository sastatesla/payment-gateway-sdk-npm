...
export class StripeProvider extends PaymentProvider {
	...
	public async initialize(config: StripeConfig): Promise<void> {
		try {
			...
			throw APIError({
				message: `[StripeProvider Initialize] ${err.message}`
			})
		}
	}
	...
	constructor(config: StripeConfig) {
		...
		try {
			...
			throw APIError({
				message: `[StripeProvider Constructor] ${err.message}`
			})
		}
	}
	...
	public async charge(input: ChargeInput): Promise<ChargeResult> {
		...
		try {
			const orderRequest = {
				...
				customer_details: {
					customer_id:
						input.metadata?.customerId || `cust_${Date.now()}`,
			...
			const order = await this.stripe.orders.create(orderRequest)
			return {
				...order,
	...
	public async getPaymentStatus(paymentId: string) {
		...
		try {
			const payment = await this.stripe.payments.get(paymentId)
			return {...payment}
		} catch (err: any) {
			throw APIError({
	...
	public async listUserPayments(
		...
		try {
			...
			const allPayments = await this.stripe.orders.list(params)
		...
		try {
			...
			if (filters?.status) {
				results = results.filter(
					(p: any) => p.order_status === filters.status
				)
			}
			return results.map((p: any) => ({
				id: p.order_id,
	...
	public async getSettlementDetails(
		...
		try {
			const settlement = await this.stripe.settlements.get(settlementId)
			return {
				...settlement,
	...
	public async getRefundStatus(refundId: string): Promise<RefundResult> {
		...
		try {
			const refund = await this.stripe.refunds.get(refundId)
			return {
				...refund,
				amount: Number(refund.refund_amount) * 100,
				createdAt: new Date(refund.refund_time).toISOString()
	...
	public async fetchVirtualAccountDetails(accountId: string) {
		try {
			return await this.stripe.virtualAccounts.get(accountId)
		} catch (err: any) {
			throw APIError({
